...
const faderNorm = (db) => clamp((Number(db) + 90) / 100, 0, 1);
...
export class M32Driver extends EventEmitter {
  constructor({ host, port = DEFAULT_OSC_PORT, localPort = DEFAULT_LOCAL_PORT, refreshMs = 9000, autoRefresh = true, meterIntervalMs = 1000 } = {}) {
    super();
    if (!host) throw new Error('M32 host is required');
    this.host = host;
    this.port = port;
    this.localPort = localPort;
    this.refreshMs = Math.max(3000, refreshMs);
    this.autoRefresh = autoRefresh;
    this.meterIntervalMs = Math.max(1000, meterIntervalMs);
    this.socket = null;
  ...
  async refresh() {
    this.#requireSocket();
    await this.#sendAndWait('/xremote', []);
    for (const address of ['/info', '/status', '/config', '/-stat/chfaderbank', '/-stat/grpfaderbank', '/-stat/sendsonfader']) {
      await this.#sendAndWait(address, []);
    }
    await this.subscribeMeters(true);
  }

  async subscribeMeters(enable = true) {
    for (const group of ['/meters/0', '/meters/6', '/meters/7', '/meters/12']) {
      await this.send('/meters', [group, enable ? 1 : 0]);
    }
  }

  async setChannelFader(channel, value) {
    return this.set(channelPath(this.#ch(channel), 'fdr'), faderNorm(value));
  }
  ...
  async setChannelHpf(channel, frequency) { return this.set(channelPath(this.#ch(channel), 'eq/lo/f'), clamp(frequency, 20, 20000)); }
  async setChannelEqBand(channel, band, { on, type, frequency, gain, q } = {}) {
    ...
    if (frequency !== undefined) jobs.push(this.set(`${base}/f`, clamp(frequency, 20, 20000)));
    if (gain !== undefined) jobs.push(this.set(`${base}/g`, clamp(gain, -15, 15)));
    if (q !== undefined) jobs.push(this.set(`${base}/q`, clamp(q, 0.1, 10)));
    return Promise.all(jobs);
  }

  async setBusSend(channel, bus, { level, on, pan, prepost } = {}) {
    this.#ch(channel); this.#bus(bus);
    const base = channelPath(channel, `mix/${String(bus).padStart(2, '0')}`);
    const jobs = [];
    if (level !== undefined) jobs.push(this.set(`${base}/level`, faderNorm(level)));
    if (on !== undefined) jobs.push(this.set(`${base}/on`, !!on));
  ...
  async setBusFader(bus, value) { return this.set(busPath(this.#bus(bus), 'fdr'), faderNorm(value)); }
  async setBusMute(bus, on) { return this.set(busPath(this.#bus(bus), 'on'), !!on); }
  async setMatrixFader(matrix, value) { return this.set(matrixPath(this.#matrix(matrix), 'fdr'), faderNorm(value)); }
  async setDcaFader(dca, value) { this.#dca(dca); return this.set(`/dca/${String(dca).padStart(2, '0')}/fdr`, faderNorm(value)); }
  async setDcaMute(dca, on) { this.#dca(dca); return this.set(`/dca/${String(dca).padStart(2, '0')}/on`, !!on); }
  async setMainFader(value) { return this.set('/main/st/fader', faderNorm(value)); }
  async setMainMute(on) { return this.set('/main/st/on', !!on); }
  async setMonoFader(value) { return this.set('/main/m/fader', faderNorm(value)); }
  async setParam(address, ...args) { return this.set(address, ...args); }

  async #sendAndWait(address, args) {
  ...
  #onPacket(message, rinfo) {
    ...
    const event = { timestamp: this.lastRxAt, address: packet.address, args: packet.args, peer: `${rinfo.address}:${rinfo.port}` };
    this.rawFeedback.push(event);
  ...
  #beginTimers() {
    if (this.autoRefresh && !this.refreshTimer) {
      this.refreshTimer = setInterval(() => this.refresh().catch((e) => this.emit('error', e)), this.refreshMs);
    }
    if (!this.meterTimer) {
      this.meterTimer = setInterval(() => this.subscribeMeters(true).catch((e) => this.emit('error', e)), this.meterIntervalMs);
    }
  }

  #requireSocket() { if (!this.socket || !this.connected) throw new Error('M32 is not connected'); }
