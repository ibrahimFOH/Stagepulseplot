(() => {
  ...
  const categoryPhotos={instrument:photos['Vokal mikrofonu'],pa:photos['Line Array L'],console:photos['Midas M32'],stage:photos['Line Array L'],light:photos['Midas M32'],production:photos['Vokal mikrofonu']};
  ...
  function urlFor(name,category){if(photos[name])return photos[name];const n=name.toLocaleLowerCase('tr-TR');if(n.includes('mikrofon'))return photos['Vokal mikrofonu'];if(n.includes('gitar'))return n.includes('bas')?photos['Bas gitar']:(n.includes('akustik')?photos['Akustik gitar']:photos['Elektro gitar']);if(['keman','viyola','çello','kontrbas','kemençe'].some(x=>n.includes(x)))return photos['Keman'];if(n.includes('piyano')||n.includes('klavye'))return photos['Piyano'];if(n.includes('davul')||n.includes('drum'))return photos['Davul seti'];if(n.includes('line array'))return photos['Line Array L'];if(category&&categoryPhotos[category])return categoryPhotos[category];return null;}
  function apply(container){container.querySelectorAll('.item').forEach(item=>{const name=nameFromItem(item);const icon=item.querySelector('.icon');const m=icon?.className.match(/icon-(\w+)/);const url=urlFor(name,m?.[1]);if(!icon||!url||icon.dataset.photoUrl===url)return;icon.dataset.photoUrl=url;icon.innerHTML=`<img class="equipment-photo" src="${url}" alt="${name.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" loading="lazy" referrerpolicy="no-referrer">`;});container.querySelectorAll('.obj').forEach(obj=>{const icon=obj.querySelector('.obj-icon');const name=nameFromObject(obj);if(!icon||!name)return;const url=urlFor(name);if(!url||icon.dataset.photoUrl===url)return;icon.dataset.photoUrl=url;icon.innerHTML=`<img class="equipment-photo stage-photo" src="${url}" alt="${name.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" loading="lazy" referrerpolicy="no-referrer">`;});}
  const style=document.createElement('style');style.textContent='.equipment-photo{display:block;width:100%;height:100%;object-fit:contain;border-radius:8px;background:#fff}.icon .equipment-photo{padding:3px;box-sizing:border-box}.icon{overflow:hidden}.obj-icon{overflow:hidden;border-radius:7px}.stage-photo{padding:4px;box-sizing:border-box;background:#fff}.item .icon{background:#20262d}';document.head.appendChild(style);
