...
root.querySelectorAll('.obj').forEach(obj=>{const icon=obj.querySelector('.obj-icon');if(!icon)return;const name=nameOfObj(obj);const item=[...document.querySelectorAll('#library .item')].find(e=>nameOfItem(e)===name);const cat=item?.querySelector('.icon')?.className.match(/icon-(\w+)/)?.[1];if(disabled.has(cat))icon.dataset.photoMode='disabled';});
}
