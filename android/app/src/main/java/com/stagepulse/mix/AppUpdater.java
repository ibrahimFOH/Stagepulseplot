...
/** In-app GitHub release updater without embedded credentials. */
public final class AppUpdater {
    ...
    new AsyncTask<Boolean, Void, UpdateInfo>() {
      ...
      @Override protected UpdateInfo doInBackground(Boolean... ignored) {
        try {
          ...
          if (apkUrl == null || apkUrl.isEmpty()) throw new IllegalStateException("Release APK bulunamadı");
      ...
      @Override protected void onPostExecute(UpdateInfo info) {
        if (info == null) {
          if (error != null && notifyWhenCurrent) Toast.makeText(activity, "Güncelleme kontrolü başarısız", Toast.LENGTH_SHORT).show();
          return;
        }
  ...
  private final class DownloadTask extends AsyncTask<String, Void, File> {
    @Override protected File doInBackground(String... urls) {
      File part = new File(activity.getExternalCacheDir() != null ? activity.getExternalCacheDir() : activity.getCacheDir(), "stagepulsemix-update.apk.part");
      try {
        ...
        try (InputStream in = new BufferedInputStream(c.getInputStream()); FileOutputStream fos = new FileOutputStream(part, false)) {
          byte[] buf = new byte[16384]; int n;
          while ((n = in.read(buf)) != -1) fos.write(buf, 0, n);
        }
        c.disconnect();
        File apk = new File(part.getParentFile(), "stagepulsemix-update.apk");
        if (apk.exists()) apk.delete();
        if (!part.renameTo(apk)) throw new IllegalStateException("APK dosyası hazırlanamadı");
        return apk;
    ...
    @Override protected void onPostExecute(File apk) {
      ...
      try {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && !activity.getPackageManager().canRequestPackageInstalls()) {
          ...
          Toast.makeText(activity, "APK kurulumu için izin gerekli", Toast.LENGTH_LONG).show();
          return;
        }
  ...
  private static int compare(String a, String b) {
    int[] x=parse(a), y=parse(b); for(int i=0;i<3;i++) if(x[i]!=y[i]) return Integer.compare(x[i],y[i]); return 0;
  }
